    ...
    return (
        <DefaultLayout>
            <div className="mx-auto max-w-2xl">
                <div className="mb-6">
                    <h2 className="text-3xl font-bold tracking-tight text-black dark:text-white">Add Get Together Type</h2>
                    <p className="text-muted-foreground">Used in the app for colored icon cards (e.g. Wedding, Birthday).</p>
                </div>
                <form onSubmit={handleSubmit} className="rounded-lg border border-stroke bg-white p-6 shadow dark:border-strokedark dark:bg-boxdark">
                    <div className="space-y-4">
                        <div>
                            <label className={labelClass}>ID (unique, lowercase, e.g. baby_shower) *</label>
                            <input type="text" value={formData.id} onChange={e => handleChange('id', e.target.value.toLowerCase().replace(/\s/g, '_'))} required className={inputClass} placeholder="e.g. baby_shower" />
                        </div>
                        <div>
                            <label className={labelClass}>Name *</label>
                            <input type="text" value={formData.name} onChange={e => handleChange('name', e.target.value)} required className={inputClass} placeholder="e.g. Baby Shower" />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className={labelClass}>Image (optional, overrides emoji)</label>

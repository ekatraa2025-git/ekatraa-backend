...
export default function EditEInviteTemplatePage() {
    ...
    useEffect(() => {
        fetch(`/api/admin/e-invites/templates/${id}`)
            .then((r) => r.json())
            .then((data) => {
                if (data?.error) {
                ...
                setForm({
                    section_key: data.section_key || 'wedding_cards',
                    title: data.title || '',
                    subtitle: data.subtitle || '',
                    thumbnail_url: data.thumbnail_url || '',
                    preview_url: data.preview_url || '',
                    template_type: data.template_type || 'image',
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        const res = await fetch(`/api/admin/e-invites/templates/${id}`, {
            method: 'PATCH',
    ...
    return (
        <DefaultLayout>
            <Card className="max-w-3xl">
                <CardHeader>
                    <CardTitle>Edit E-Invite Template</CardTitle>
                </CardHeader>
                <CardContent>
                        ...
                        <div>
                            ...
                            <div className="mt-2 flex items-center gap-3">
                                ...
                                <span className="text-xs text-muted-foreground">
                                    Replace with image/video template file
                                </span>
                            </div>
                        </div>
